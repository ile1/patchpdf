...
async function loadPdf(bytes, name) {
  ...
  renderLines();
  setPreview("original");
  els.statusLines.textContent = `${snap.pageCount} page(s) · ${state.lines.length} text runs · all processing stays in your browser`;
  setMsg(null);
...
async function exportDocx() {
  ...
  try {
    ...
    setMsg(
      `DOCX: ${result.pages} page(s), ${result.lines} line(s).` +
